gameModule
	.factory('RoulHisToryManagerForLobby', ['RoulTableServ', 'BigSmallTableServ', 'RedBlackTableServ', 'EvenOldTableServ', '$timeout', 'CanvasServ',
		function(RoulTableServ, BigSmallTableServ, RedBlackTableServ, EvenOldTableServ, $timeout, CanvasServ) {
			...
			function RoulHistoryResultManger(type, tableid, viewid) {
				if (tableid) {
			...
			function ShowTable() {
				...
				while (index < roadView.length) {
					if (roadLoadView[index] == null) {
						...
						rtable.Initialize(parmas[type],viewcanvasId);
