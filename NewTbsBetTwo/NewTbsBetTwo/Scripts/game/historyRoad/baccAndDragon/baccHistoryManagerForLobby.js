gameModule
	.factory('LobbyHisToryManager', ['BaccTableServLobby', '$timeout', 'CanvasServ',
		function(BaccTableServLobby, $timeout, CanvasServ) {
			...
			function BaccaratHistoryResultManger(tableid) {
				if(tableid){
					tablecanvasId=tableid;
				}
			parmas = {
						strTable: {
							'rows': 6,
				...
				strPearlTable = BaccTableServLobby;
				roadView = new Array();
			...
			function SetViewId(gametype,viewid){
				gameType = gametype;
				if (viewid) {
					viewcanvasId = viewid;
				}
				strPearlTable.SetViewId(gametype,viewcanvasId);
			}
			/*
			 * 服务器返回字符串分割
			 @ number 向前推进个数
			...
			function ShowTable() {
				...
				while (index < roadView.length) {
					if (roadLoadView[index] == null) {
						var rtable = roadView[index];
						roadLoadView[index] = rtable;
						rtable.Initialize(parmas, gameType);
