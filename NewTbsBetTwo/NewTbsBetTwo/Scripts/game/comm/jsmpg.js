(function(window) {
	...
	jsmpeg.prototype.destory = function() {
		if(this.client){
			this.client.close();
			this.client=null;
		}
		...
		if(this.externalFinishedCallback){
			this.externalFinishedCallback = null;
		}
		if (this.customIntraQuantMatrix) {
			this.customIntraQuantMatrix = null;
		}
		if (this.customNonIntraQuantMatrix) {
			this.customNonIntraQuantMatrix = null;
		}
		if (this.blockData) {
			this.blockData = null;
		}
		if (this.zeroBlockData) {
			this.zeroBlockData = null;
		}
		if(this.buffer){
			this.buffer=null;
		}
		if (this.nextPictureBuffer) {
			this.nextPictureBuffer = null;
		}
	...
	//webscoket第一次获取数据初始化，receiveSocketMessage
	...
	//webscoket传回数据，initSocketClient
	...
	jsmpeg.prototype.decodeBlock = function(block) {
		...
		while (true) {
			...
			if(quantMatrix && quantMatrix[dezigZagged]){
			...
			if (PREMULTIPLIER_MATRIX && PREMULTIPLIER_MATRIX[dezigZagged]) {
