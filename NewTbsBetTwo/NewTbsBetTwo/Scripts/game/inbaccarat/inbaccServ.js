gameModule
	.factory('InBaccServ', ['$state', 'GameSendServ', 'GameCommFun', 'ChipSelectManager', 'BaccHisToryManager', '$rootScope', '$timeout', 'CommFun', 'CommandServ', 'LimitServ',
		function($state, GameSendServ, GameCommFun, ChipSelectManager, BaccHisToryManager, $rootScope, $timeout, CommFun, CommandServ, LimitServ) {
			...
			var serverdata = {
				...
				chips: null, //选中5个筹码
				showpk: false, //是否显示牌
				playerIn: false, //闲保险是否可以投注
				bankerIn: false, //庄保险是否可以投注
				pkresult: [], //牌结果，闲，庄
			...
			function OnBet(index) {
				...
				if ((serverdata.playerIn || serverdata.bankerIn) && index < 5) {
					//CommFun.ShowMessage($rootScope.MessageType.BetMessage, 4); //位置已达最大投注
					return;
				}
				if ((serverdata.playerIn && index != 7) || (serverdata.bankerIn && index != 8)) {
					return;
				}
			...
			function SetInsuranceMaxLimit(insurance, odd) {
				if (insurance == 1) {
					maxbetlimit[7] = Math.floor(serverdata.bet[0] / odd);
				}
				if (insurance == 2) {
					maxbetlimit[8] = Math.floor(serverdata.bet[1] / odd);
				}
					...
					case $rootScope.GameForBaccarat.StartBet: //开始下注
						...
						if (startBet.Insurance > 0) {
							...
							if (startBet.Insurance == 1) {
								serverdata.bankerIn = false;
								serverdata.playerIn = true;
								CommFun.ShowMessage($rootScope.MessageType.GameStatus, 2); //显示闲保险投注提示框;
							}
							if (startBet.Insurance == 2) {
								serverdata.playerIn = false;
								serverdata.bankerIn = true;
								CommFun.ShowMessage($rootScope.MessageType.GameStatus, 3); //显示庄保险投注提示框;
							}
							SetInsuranceMaxLimit(startBet.Insurance, startBet.Odds);
						}
						TragetPane(1);
						serverdata.table.status = 2; //桌台状态
						GameCommFun.SetTimeEle(startBet.BetTime);
